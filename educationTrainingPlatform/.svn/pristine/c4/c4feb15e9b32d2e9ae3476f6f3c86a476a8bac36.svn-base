...
var calendar = function(calendarId,markdate){
    ...
    c.getcalendar = function(year,month){
        ...
        //星期是从0开始到6结束的，小于等于上个月最大星期的日期则是上个月的日期
            ...
            if(g <= lastday){
                ...
                if(thisyear == year && thismonth == month && today == t){
                    str +='<td class="redTr '+ markClass + '" onclick="calendarClick(this)" onmouseover="mouseIn(this)" onmouseout="mouseOut(this)" value="'+ t + '">'+ t++ +'</td>';
                //假如不是当前月份则默认1为高亮日期
                }/*else if((thisyear != year && t == 1) || (thismonth != month && t == 1)){
                    str +='<td class="redTr">'+ "t"+t++ +'</td>';
                }*/else{
                //该月份的普通日期
                    str +='<td class="blueTr '+ markClass + '" onclick="calendarClick(this)" onmouseover="mouseIn(this)" onmouseout="mouseOut(this)" value="'+ t + '">'+ t++ +'</td>';
                }
            }
        }
        str += '</tr>';
        ...
        for(var i=1;i<=5;i++){
            ...
            for(var j=0;j<7;j++){
                var markClass = "";
                for(var mark =0;mark<Maxdate;mark++){
                    if(remainlastday == markdate[mark]){
                        markClass = "haveMark";
                        break;
                    }
                }
                if(remainlastday <= Maxdate){
                    ...
                    str +='<td class="redTr '+ markClass + '" onclick="calendarClick(this)" onmouseover="mouseIn(this)" onmouseout="mouseOut(this)" value="'+ remainlastday + '">'+ remainlastday++ +'</td>';
                    }else{
                        /*if((thisyear != year && remainlastday == 1) || (thismonth != month && remainlastday == 1)){
                            ...
                            str +='<td class="blueTr '+ markClass + ' " onclick="calendarClick(this)" onmouseover="mouseIn(this)" onmouseout="mouseOut(this)" value="'+ remainlastday + '">'+ remainlastday++ +'</td>';
                        /*}*/
                    }
                }else{
    ...
    c.init = function(){
        ...
        loadStyles(".calendarDiv {width:461px;text-align:center;}\.event-over{opacity: 0.6;}\
            .calendar-table{width:460px;height:300px;}\
            .haveMark{background-image: url('../images/calenderpic.png');background-repeat: no-repeat;background-position: 90% 10%;}\
