package com.foxera.background.mongodao.impl;
...
import com.foxera.background.mongodao.MongoDBDao;
...
public class MongoDBDaoImpl implements MongoDBDao {
	...
	/**
	 * MongoClient的实例代表数据库连接池，是线程安全的，可以被多线程共享，客户端在多线程条件下仅维持一个实例即�?
	...
	/**
	 * 
	 * 方法名：getMongoDBDaoImplInstance 作�?：zhouyh 创建时间�?014-8-30 下午04:29:26
	 * 描述：单例的静�?工厂方法
	 * 
	 * @return
	 */
...
//				}
				for (Entry<String, String> entry : keyValues.entrySet()) {
					//按条件删�?格式为keyValues.put("createDate", "{$gt:2016-12-28}"):表示删除createDate大于2016-12-28
...
//				}
				for (Entry<String, String> entry : keyValues.entrySet()) {
					//按条件查�?格式为keyValues.put("createDate", "{$gt:2016-12-28}"):表示查询createDate大于2016-12-28
					String pvalue=entry.getValue();
					if(pvalue.startsWith("logic{")&&pvalue.endsWith("}")&&pvalue.contains(":")){
						pvalue=pvalue.replace("logic{", "").replace("}", "");
						String[] logic=pvalue.split(":");
...
//		 try {
//		 db = mongoClient.getDB(dbName); // 获取数据库实�?
